    ...
    <Card className="surface-panel app-surface rounded-[30px] border border-border/70 text-foreground shadow-[var(--surface-shadow-soft)]">
      <CardHeader className="space-y-3">
        ...
        <CardDescription className="text-sm leading-6 text-muted-foreground">
          当前会话固定绑定 {getModelDisplayName(model)}。切换模型会新建会话，不混上下文。
        </CardDescription>
      </CardHeader>
      ...
      <CardContent className="space-y-4 pb-4">
        ...
        <div className="rounded-[22px] border border-border/70 bg-background/60 p-3.5">
          ...
          <ul className="mt-2 flex flex-col gap-2 text-sm leading-6 text-muted-foreground">
            ...
            <li>
              {isDefaultGeneralModel
                ? "当前就是默认通用模型，适合初次使用和主路径演示。"
                : `${getModelDisplayName(defaultGeneralModel)} 更适合作为默认入口，不确定就回到它。`}
            </li>
          </ul>
        </div>

        <div className="space-y-2.5">
          <div className="text-sm font-medium text-foreground">适合场景</div>
          <ul className="flex flex-col gap-2.5 text-sm leading-6 text-muted-foreground">
