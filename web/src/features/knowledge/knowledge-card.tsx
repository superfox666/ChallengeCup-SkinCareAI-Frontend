import type { KnowledgeCardData } from "@/content/knowledge-cards"
import { Badge } from "@/components/ui/badge"
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card"
import { knowledgeCategoryLabels } from "@/content/knowledge-cards"
...
export function KnowledgeCard({ item }: KnowledgeCardProps) {
  return (
    <Card className="rounded-[28px] border border-white/8 bg-[linear-gradient(180deg,rgba(16,29,54,0.96),rgba(11,19,35,0.9))] text-white shadow-[0_24px_60px_rgba(0,0,0,0.14)]">
      <CardHeader>
        <div className="flex items-start justify-between gap-3">
          <CardTitle className="text-[1.2rem] leading-[1.2]">{item.title}</CardTitle>
